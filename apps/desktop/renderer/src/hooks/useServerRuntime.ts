...
export function useServerRuntime(serverId: string | null): ServerRuntime {
  ...
  const refreshStatus = useCallback(async (id: string): Promise<void> => {
    try {
      ...
      if (serverIdRef.current !== id) return;
  ...
  const run = useCallback(
    async (action: () => Promise<{ error: unknown } | { ok: boolean }>): Promise<void> => {
      ...
      try {
        ...
        if (serverIdRef.current === actionServerId) {
          await refreshStatus(actionServerId);
        }
      } catch (err) {
        setError(err instanceof Error ? err.message : String(err));
      }
  ...
  const stop = useCallback(
    () => run(() => api.stopServer(serverId as string)),
    [run, serverId],
  );
  ...
  const forceKill = useCallback(
    () => run(() => api.forceKillServer(serverId as string)),
    [run, serverId],
  );
